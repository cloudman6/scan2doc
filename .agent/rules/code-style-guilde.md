...
- Every long-running operation must:
  ...
  - log status updates
- 所有日志信息必须使用 consola，不能使用 console
---
...
- **If you starts a development server during debugging, remember to shut it down after testing is complete**
...
# 行为准则
1. 在收到任何复杂任务时，必须先输出一个 [PLAN] 模块。
2. 在 [PLAN] 中列出：受影响的文件、潜在风险、修改步骤。
3. 必须在用户输入 "Proceed" 后才能开始改动代码。